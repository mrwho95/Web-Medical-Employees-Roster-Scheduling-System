...
<body id="page-top">
  ...
  <nav class="navbar navbar-expand navbar-dark bg-dark static-top">
    ...
    <div id="wrapper">
      ...
      <ul class="sidebar navbar-nav">
        ...
        <div id="schedulerEditModal" class="modal fade">  
          <div class="modal-dialog">  
            <form method="post" id="scheduler_Editform"> 
              <div class="modal-content">  
                <div class="modal-header">
                  <h4 class="modal-title">Edit New Scheduler</h4>  
                  <button type="button" class="close" data-dismiss="modal">&times;</button>  
                </div>  
                <div class="modal-body">  
                  <label>Full Name</label>  
  ...
  {  
   ...
   "orderable":true,  
    ...
    //Edit scheduler Submission
    $(document).on('submit', '#scheduler_Editform', function(event){  
     event.preventDefault();

     var Full_Name = $('#Editfullname').val();  
     var Gender = $('#Editgender').val();  
     var Phone_Number = $('#Editphonenumber').val();
     var Email_Address = $('#EditinputEmail').val();
     var Department = $('#Editdepartment').val();
     var Hospital = $('#Edithospital').val();  
     var user_id = $('#Edituser_id').val(); 

     console.log(user_id);

     if(Full_Name != '' && Gender != '' && Phone_Number != '' && Department != '' && Hospital != '' && Email_Address != '')
     {  
      $.ajax({  
       url:"<?php echo base_url() . 'admin2/edit_scheduler_action'?>",  
       method:'POST',  
       data:new FormData(this),  
       contentType:false,  
       processData:false,  
       ...
       {  
        ...
        alert(data);  
        $('#scheduler_Editform')[0].reset();  
        $('#schedulerEditModal').modal('hide');  
        dataTable.ajax.reload();  
    ...
    $(document).on('click', '.update', function(){  
     ...
     $.ajax({  
      ...
      {  
       ...
       $('#Edithospital').val(data.hospital);  
