   ...
   <div id="wrapper">
    ...
    <ul class="sidebar navbar-nav">
      ...
      <div id="hospitalModal" class="modal fade">  
       ...
       <!--Edit hospital-->
      <div id="hospitalEditModal" class="modal fade">  
        <div class="modal-dialog">  
          <form method="post" id="hospital_Editform"> 
            <div class="modal-content">  
              <div class="modal-header">
                <h4 class="modal-title">Add New Hospital</h4>  
                <button type="button" class="close" data-dismiss="modal">&times;</button>
    ...
    $(document).on('click', '.update', function(){  
     ...
     $.ajax({  
      ...
      success:function(data)  
      {  
       $('#hospitalEditModal').modal('show');  
       $('#Edithname').val(data.hname);  
       $('#Editlocation').val(data.location);
       $('#Edithospital_type').val(data.hospital_type);    
    ...
    //Edit hospital Submission
      $(document).on('submit', '#hospital_Editform', function(event){  
       event.preventDefault();  
       var HName = $('#Edithname').val();  
       var Location = $('#Editlocation').val();  
       var Type = $('#Edithospital_type').val();

       if(HName != '' && Location != '' && Type != '')
       {  
        $.ajax({  
         url:"<?php echo base_url() . 'admin2/update_hospital_action'?>",  
         method:'POST',  
         data:new FormData(this),  
         contentType:false,  
         processData:false,  
         ...
         {  
          alert(data);  
          $('#hospital_Editform')[0].reset();  
          $('#hospitalEditModal').modal('hide');  
          dataTable.ajax.reload();  
        }  
      });  
      }  
